                            ...
                            isSending ? <Loading/> :
                                <>
                                    <form onSubmit={(e) => e.preventDefault()}>
                                        <ul>
                                            <FormHelperText
                                                error={true}>{message ? t(message.substring(0, message.indexOf(" :"))) + email : ""}</FormHelperText>
                                            <li>
                                                <label className="label" htmlFor="">{t("Phone number")}</label>
                                                <InputMask mask="+\9\98(99)999 99 99" required onChange={(e) => {
                                                    setEmail(e.target.value
                                                        .replaceAll(" ", "")
                                                        .replaceAll("(", "")
                                                        .replaceAll(")", ""));
                                                    setMessage("")
                                                }} className="input-text" type="phone"
                                                           placeholder={t("Input phone number")}/>
                                            </li>
                                            <li>
                                                <button onClick={sendRequest}
                                                        className="btn-default">{t("Send")}</button>
                                            </li>
                                        </ul>
                                    </form>
                                </>
