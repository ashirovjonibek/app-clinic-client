...
const Appeals = ({
    ...
    const getStatus = (a) => {
        switch (a) {
            case "CREATED":
                return {text: "Yangi murojaat", color: "blue"};
            case "INPROCESS":
                return {text: "Jarayonda", color: "yellow"};
            case "DENIED":
                return type !== types?.newAppFromApplicant ? {text: "Jarayonda", color: "yellow"} : {
                    text: "Qaytarildi",
                    color: "red"
                };
            case "COMPLETED":
                return {text: "Bajarilgan", color: "green"};
            default:
                return {text: "No date", color: "yellow"}
        }
    }
                ...
                <Card className="rounded card-appeals border"
                    ...
                    <Row gutter={24}>
                        {types.newAppFromApplicant !== type && <Col xs={12} sm={12} md={12} lg={12}>
                            <Avatar className="m-0 p-0"
                                    src={appeal?.applicant?.image && <Image src={API_URL + appeal?.applicant?.image}
                                                                            style={{
                                                                                width: "100%",
                                                                                height: "45px",
                                                                                objectFit: 'cover'
                        ...
                        {
                            types.newAppFromApplicant !== type ? appeal?.deadLineDate &&
                                <Col className="text-end" Col xs={12} sm={12} md={12} lg={12}>
                                    <p style={{width: "100%", textAlign: "right", marginTop: "4px"}}><span
                                        style={{fontWeight: 600}}>Ko'rib chiqish muddati: </span>{getDay(appeal?.deadLineDate)} kun
                                    </p>
                                </Col> : ""
                        }
                    ...
                    <Row className="border rounded" gutter={24}>
                        <Col className="border p-3" lg={24} md={24} sm={24} xs={24}>
                            ...
                            {
                                appeal?.description?.length > 2000 &&
                    ...
                    </Row>
                    {
                        types.newAppFromApplicant !== type && <>
                            ...
                            {checkedBy &&
                            <AnswerListener type={type} setDocOpen={setDocOpen} setDoc={setDoc} item={checkedBy}
                                            answer={answer}/>}
