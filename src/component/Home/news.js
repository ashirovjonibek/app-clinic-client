...
const NewsPaper=()=>{
    ...
    useEffect(()=>{
        axios({
            ...
            for (let i = res?.data?.items.length-4; i < res?.data?.items.length; i++) {
    ...
    return(
        <div className="container-fluit">
           <div className="new-container">
                   <Swiper className="news-swipe" style={{'--swiper-navigation-color': '#fff','--swiper-pagination-color': '#fff'}} zoom={true} navigation={true}
                       autoplay={{
                           delay: 3000,
                       ...
                       loop={true}>
                       {
                           news&&news.map((item,i)=>
                               <SwiperSlide key={i} className="news-swipe-slide">
                                   <div onClick={()=>targetLink(item.id)} className="general-news">
               ...
               <div className="other-news">
                   {
                       right&&right.map((item,i)=>
                           <div key={i} onClick={()=>targetLink(item.id)} className="news-card">
                               <div className="news-card-container">
                                   <div className="news-card-imd">
                                       <img src={"https://proacademy.uz/postfiles/documents"+item?.img_link} alt="img" />
                                   </div>
                                   <div className="news-card-content">
                                       <p className="other-news-title">{getLangContent(item?.title)}</p>
                                       <p className="other-news-short">{getLangContent(item?.short_content)}</p>
