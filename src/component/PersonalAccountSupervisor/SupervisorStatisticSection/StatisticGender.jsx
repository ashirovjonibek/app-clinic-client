import React from "react";

const StatisticGender = () => {
    return (
        <div className="statistic-gender">
            <div className="statistic-section-item-title">
                <h5>По гендерном признаку заявителей</h5>
            </div>
            <div className="statistic-section-item-body">
                <div className="statistic-item-line">
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">0</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">20</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">40</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">60</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">80</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">100</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">120</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">140</div>
                    </div>
                    <div className="item-line">
                        <div className="line"></div>
                        <div className="item">160</div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Ферганская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '390px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '270px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Наманганская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '190px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '70px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Андижанская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '490px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '370px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Сурхандарьинская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '290px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '170px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Кашкадарьинская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '90px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '270px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Хорезмская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '290px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '170px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Бухарская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '150px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '370px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Навоинская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '120px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '190px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Джизакская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '290px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '170px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Сырдаьинская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '110px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '220px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Ташкенсткая</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '150px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '470px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Самаркандская</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '290px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '170px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="statistic-item">
                    <div className="statistic-item-name">Ташкент</div>
                    <div className="statistic-item-progres-section">
                        <div className="item-progres">
                            <div className="progres-women" style={{ width: '290px' }}></div>
                            <span className="item">123</span>
                        </div>
                        <div className="item-progres">
                            <div className="progres-men" style={{ width: '370px' }}></div>
                            <span className="item">123</span>
                        </div>
                    </div>
                </div>
                <div className="gender-inform">
                    <div className="gender">
                        <div className="gender-item">
                            <input type="checkbox" className="checkbox" />
                            <div className="categories-color" style={{ backgroundColor: '#78BAF3' }}></div>
                            <p>Мужщины</p>
                        </div>
                        <div className="gender-item">
                            <input type="checkbox" className="checkbox" />
                            <div className="categories-color" style={{ backgroundColor: '#F57670' }}></div>
                            <p>Женщины</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default StatisticGender;
