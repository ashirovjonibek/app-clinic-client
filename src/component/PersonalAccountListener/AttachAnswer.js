                    ...
                    <div className="content">
                        <div className="request-content-title">
                            ...
                            <div className="request-content-title-date">
                                ...
                                <div
                                >
                                    <Tag color={getDayDeadline(item?.application?.deadLineDate)>10?"green":getDayDeadline(item?.application?.deadLineDate)<=10&&getDayDeadline(item?.application?.deadLineDate)>5?"yellow":"red"} style={{marginLeft:"5px"}}>
                                ...
                                <Col className="p-3" xs={24} sm={24} md={18} lg={18}>
                                    ...
                                    <Row>
                                        {item?.application?.attachmentsId && <Col lg={3} md={4} sm={8} xs={24} className="p-1">
                                            <Tag onClick={() => { setDoc(API_URL + '/attach/' + item?.application?.attachmentsId[0]); setDocOpen(true) }} style={{ width: "100%", cursor: 'pointer' }} className="text-center p-2">
                                                <FileOutlined style={{ fontSize: "20px" }} />
                                            </Tag>
                                        </Col>}
                                        {item?.application?.video && <Col lg={3} md={4} sm={8} xs={24} className="p-1">
                                            <Tag onClick={() => { setMOdalItem({ url: API_URL + item?.application?.video, type: "v" }); setOpen(true) }} style={{ width: "100%", cursor: 'pointer' }} className="text-center p-2">
                                                <VideoCameraOutlined style={{ fontSize: "20px" }} />
                                            </Tag>
                                        </Col>}
                                        {item?.application?.audio && <Col lg={3} md={4} sm={8} xs={24} className="p-1">
                                            <Tag onClick={() => { setMOdalItem({ url: API_URL + item?.application?.audio, type: "va" }); setOpen(true) }} style={{ width: "100%", cursor: 'pointer' }} className="text-center p-2">
                        ...
                        <>
                            ...
                            <Tag style={{ width:"100%",marginTop:"5px",marginBottom:"5px",fontSize:"16px",padding:"5px"}} color={"orange"}>{item?.answer?.deniedMessage}</Tag>
                        </>
                        <hr/>
